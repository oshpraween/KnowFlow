...
import { useNavigate } from 'react-router-dom';
import Alert from './Alert';
import { Users, BookOpen, Settings, Trash2, UserMinus, Mail, Shield, Info } from 'lucide-react';
...
const GroupManagement = ({ group, currentUser, onUpdate }) => {
  ...
  const confirmAction = async () => {
    try {
      ...
      if (showConfirmModal.type === 'post') {
        await axios.delete(
          `http://localhost:8081/api/groups/${group.id}/posts/${showConfirmModal.id}?userId=${currentUser.id}`,
          { headers: { 'Authorization': `Bearer ${token}` } }
        );
        setPosts(posts.filter(post => post.id !== showConfirmModal.id));
        showAlert('Post removed from group', 'success');
      } else if (showConfirmModal.type === 'member') {
        await axios.delete(
          `http://localhost:8081/api/groups/${group.id}/members/${showConfirmModal.id}?userId=${currentUser.id}`,
          { headers: { 'Authorization': `Bearer ${token}` } }
        ...
        setMembers(members.filter(member => member.id !== showConfirmModal.id));
        showAlert('Member removed from group', 'success');
      }
    } catch (error) {
      console.error('Error performing action:', error);
      showAlert(error.response?.data?.message || 'Failed to perform action', 'error');
    } finally {
      setShowConfirmModal({ type: null, id: null });
    }
  };
  ...
  if (!isOwner) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6 text-center">
        ...
        <p className="text-gray-600">Only the group owner can access management features.</p>
      </div>
    );
  }
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="border rounded-lg overflow-hidden">
            ...
            <div className="max-h-80 overflow-y-auto">
              {posts.length === 0 ? (
                <div className="p-4 text-center text-gray-500">No posts shared in this group yet</div>
              ) : (
                <ul className="divide-y">
                  {posts.map((post) => (
                    <li key={post.id} className="p-4 hover:bg-gray-50 transition-colors">
                      <div className="flex justify-between items-center">
                        ...
                        <button
                          onClick={() => handleRemovePost(post.id)}
                          className="flex items-center text-red-500 hover:text-red-700 transition-colors text-sm font-medium whitespace-nowrap"
                        >
